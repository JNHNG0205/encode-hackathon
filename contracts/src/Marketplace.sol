...
contract Marketplace is ReentrancyGuard {
    ...
    function buyDataset(uint256 _listingID) public payable nonReentrant {
        ...
        require(
            ...
            "Seller cannot buy their own dataset"
        );
    ...
    function displayMarketplaceDatasets()
        public
        view
    ...
    function getSoldDatasets(
        ...
        uint256[] memory soldDatasets = new uint256[](
            userListings[seller].length
        );
        uint256 count = 0;
    ...
    function getListingDetails(
        uint256 _datasetId
    ) public view returns (bool isListed, uint256 price) {
        for (uint256 i = 1; i <= listingIdCounter; i++) {
            if (listings[i].datasetID == _datasetId && listings[i].isActive) {
